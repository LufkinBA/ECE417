...
void move_theta(double theta[5]) 
{
	...
	static int steps_from_home[5] = {0, 2151, 2151, 0, 0};
	int new_steps[5], new_steps_home[5] x;

	//put theta[x] in terms of steps
	for (x = 0;x<5;x++) { 
		if (x!=1 || x!=2) {
			new_steps[x] = theta[x] * 18.06;
		} else {
			new_steps[x] = theta[x] * 23.9;
		}
	}
	
	//new_steps = new_steps - steps_from_home
	for (x=0;x<5;x++) {
		new_steps[x] = new_steps[x] - steps_from_home[x]; 
		new_steps_home[x] = new_steps[x];
	}
	
	
	//adjust for theta1 movement
	if (new_steps_home[1] > 0) {
		new_steps[2]=new_steps[2]-(theta[1]*18.06);
		new_steps[3]=new_steps[3]+(theta[1]*23.9);
		new_steps[4]=new_steps[4]-(theta[1]*23.9);
	} else {
		new_steps_home[2]=new_steps[2]+(theta[1]*18.06);
		new_steps_home[3]=new_steps[3]-(theta[1]*23.9);
		new_steps_home[4]=new_steps[4]+(theta[1]*23.9);
	}
	
	//adjust for theta2 movement
	if (new_steps_home[2] > 0) {
		new_steps[3]=new_steps[3]+(theta[2]*23.9);
		new_steps[4]=new_steps[4]-(theta[2]*23.9);
	} else {
		new_steps[3]=new_steps[3]-(theta[2]*23.9);
		new_steps[4]=new_steps[4]+(theta[2]*23.9);
	}
	...
	moverel(new_steps[0],new_steps[1],new_steps[2],new_steps[3],new_steps[4]); 
